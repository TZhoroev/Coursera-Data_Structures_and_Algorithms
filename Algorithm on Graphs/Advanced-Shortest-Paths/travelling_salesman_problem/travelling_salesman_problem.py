...
class DistPreprocessLarge:
    def __init__(self, num_nodes, adjacency, costs):
        self.n = num_nodes
        self.INFINITY = num_nodes * max_len
        self.adj = adjacency
        self.cost = costs
...
def make_graph(contracted, vertices):
    node = next(vertices)
    vertices = list(vertices)
    assert node == len(vertices)
    graph = [[max_len] * node for _ in range(node)]
    for node_idx in range(node):
        for node_jdx in range(node):
            l = contracted.query(vertices[node_idx] - 1, vertices[node_jdx] - 1)
            graph[node_idx][node_jdx] = l if l != -1 else max_len
    return graph
...
def optimal_path(graph):
    ...
    def cost_function(S, node_idx):
        ...
        d_table[node_idx][S] = min([cost_function(S | (1 << j), j) + graph[node_idx][j] for j in range(n) if (j != 0) and (S & (1 << j) == 0)])
        return d_table[node_idx][S]
    n = len(graph)
    # Create bit mask for when all the nodes are visited
    all_seen = (1 << n) - 1
    d_table = [[max_len] * all_seen for _ in range(n)]
    x = cost_function(1, 0)

    return -1 if x == max_len else x
...
if __name__ == '__main__':
    ...
    t, = read_line()
    for i in range(t):
        print(optimal_path(make_graph(ch, read_line())))
